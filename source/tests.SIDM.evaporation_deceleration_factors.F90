...
program Tests_SIDM_evaporation_deceleration_factors
  ...
  double precision                                 , dimension(2), parameter    :: x                      =[0.5d0, 0.5d0]
  double precision                                 , dimension(2), parameter    :: evaporationFactorCheck =[0.6d0, 0.6d0]
  double precision                                 , dimension(2), parameter    :: decelerationFactorCheck =[0.13228d0, 0.13228d0]
  ...
  type            (darkMatterParticleSelfInteractingDarkMatterConstant)         :: darkMatterParticleSIDM_
  ...
  do i=1,size(velocity)
     ...
     call Assert(trim(message),evaporationFactor,evaporationFactorCheck(i),relTol=1.0d-3)
     write (message,'(a,f6.1,a,f6.1,a)') "deceleration factor at [x=",x(i),"] and orbital speed [v=",velocity(i),"]"
     call Assert(trim(message),decelerationFactor,decelerationFactorCheck(i),relTol=1.0d-3)
  end do
  ! End unit tests.
